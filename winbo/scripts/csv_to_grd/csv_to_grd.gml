...
function csv_to_grd() {
	...
	//  @jujuadams
	//	https://www.gmlscripts.com/forums/viewtopic.php?id=2081
	...
	for( var _i = 0; _i < _size; _i++ ) {
		...
		if ( ( _val == _cell_delimiter_ord ) or ( _newline ) ) and ( !_in_string ) {
			...
			if ( _is_decimal ) _string = real( _string );
			_grid[# _sheet_width-1, _sheet_height-1 ] = _string;

			_string = "";
			_in_string = false;
			_is_decimal = !_force_strings;

			//TransformValue.alpha newline outside of a string triggers a new line... unsurprisingly
			if ( _newline ) {
		...
		if ( _prev_val == 92 ) and ( _val == 110 ) {
			_string = string_delete( _string, string_length( _string ), 1 ) + chr(13);
			continue;
		}
...
function epoch_month() {
	EPO_CHKMN
}

function epoch_check() {
	EPO_CHK
}