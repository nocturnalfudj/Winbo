...
///@param {real} timeMax
function ease_elastic_in(_start,_target,_relative,_time_current,_time_max) {
	if(!_relative)
		_target -= _start;
	...
	return -(_alpha * power(2,10 * (--_time_current)) * sin((_time_current * _time_max - _s) * (2 * pi) / _p)) + _start;
}