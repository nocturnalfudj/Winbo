...
if (cached_surf_w != _surf_w || cached_surf_h != _surf_h) {
	// Create or resize surfaces
	if(!surface_exists(reflection_surface)){
		reflection_surface = surface_create(_surf_w,_surf_h);
	} else {
		surface_resize(reflection_surface,_surf_w,_surf_h);
	}
	
	if(!surface_exists(underwater_surface)){
		underwater_surface = surface_create(_surf_w,_surf_h);
	} else {
		surface_resize(underwater_surface,_surf_w,_surf_h);
	}
	...
	if(!surface_exists(reflection_composite_surface)){
		reflection_composite_surface = surface_create(_surf_w,_surf_h);
	} else {
		surface_resize(reflection_composite_surface,_surf_w,_surf_h);
	}
	
	if(!surface_exists(underwater_composite_surface)){
		underwater_composite_surface = surface_create(_surf_w,_surf_h);
	} else {
		surface_resize(underwater_composite_surface,_surf_w,_surf_h);
	}
...
// Ensure surfaces exist even if dimensions didn't change
	...
	if(!surface_exists(reflection_surface)) reflection_surface = surface_create(_surf_w,_surf_h);
	if(!surface_exists(underwater_surface)) underwater_surface = surface_create(_surf_w,_surf_h);
	if(!surface_exists(water_mask_surface)) water_mask_surface = surface_create(_surf_w,_surf_h);
	if(!surface_exists(reflection_composite_surface)) reflection_composite_surface = surface_create(_surf_w,_surf_h);
	if(!surface_exists(underwater_composite_surface)) underwater_composite_surface = surface_create(_surf_w,_surf_h);
}
...
#region Underwater
	...
	surface_copy_part(underwater_surface,0,0,application_surface,_x_on_camera,_y_on_camera,_surface_copy_width,_surface_copy_height);

	#region Underwater Composite
		surface_set_target(underwater_composite_surface);
		draw_clear_alpha(c_black,0);
		...
		// Draw composite
		draw_surface_ext(underwater_composite_surface,x,y - _height_buffer,1 * o_ui.gui_scale_inverse * _camera_zoom,1 * o_ui.gui_scale_inverse * _camera_zoom,0,c_white,1);
	#endregion
#endregion
...
#region Underwater
	...
	#region Underwater Background
		...
		// Draw composite
		draw_surface_ext(underwater_background_surface,x,y - _height_buffer,1 * o_ui.gui_scale_inverse * _camera_zoom,1 * o_ui.gui_scale_inverse * _camera_zoom,0,c_white,0.3);
	#endregion
#endregion
...
#region Reflection
	if(reflection_enable){
		//Copy Region Above Volume on Application Surface
		var _surface_copy_width,_surface_copy_height;
		_surface_copy_width = _surf_w;
		_surface_copy_height = _surf_h;
		surface_copy_part(reflection_surface,0,0,application_surface,_x_on_camera,_y_on_camera,_surface_copy_width,_surface_copy_height);
	
		// Cut out mask from the copied region: dest = dest * (1 - src_alpha)
		...
		surface_set_target(reflection_composite_surface);
		draw_clear_alpha(c_black,0);
		gpu_set_colorwriteenable(false,false,false,true);
		draw_surface(water_mask_surface,0,0);
		gpu_set_colorwriteenable(true,true,true,true);
		...
		// Draw composite instead of raw reflection
		draw_surface_ext(reflection_composite_surface,x,y - _height_buffer,1 * o_ui.gui_scale_inverse * _camera_zoom,1 * o_ui.gui_scale_inverse * _camera_zoom,0,c_white,1);
	}
#endregion
